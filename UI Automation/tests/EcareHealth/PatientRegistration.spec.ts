...
test('Patient Registration – Mandatory Fields', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'Last Name *' }).fill('barle');
  await page.getByRole('textbox', { name: 'Date Of Birth *' }).fill('02-08-2002');
  await page.getByRole('combobox', { name: 'Gender *' }).click();
  await page.getByRole('listbox', { name: 'Gender *' }).getByText('male', { exact: true }).click();

  // 7. Fill mandatory Contact Info
  await page.getByRole('textbox', { name: 'Mobile Number *' }).fill('9876548987');
  await page.getByRole('textbox', { name: 'Email *' }).fill('mohan.barle+43@thinkitive.com');
