...
test.describe('User Archive and Restore Tests', () => {
  ...
  test('Archive User', async ({ page }) => {
    ...
    await page.fill('input[placeholder="Search User"]', 'Avi');
    await page.waitForTimeout(2000);
    
    // Verify user exists in active list before archiving
    await expect(page.locator('text=Snehal yadhav')).toBeVisible();
    console.log('📋 User found in active users list');
    
    // Click on three dots (action menu) for the user
    await page.click('tr:has-text("Avi yadhav") button[aria-label="more"], tr:has-text("Avi yadhav") button:has([data-testid="MoreVertIcon"])');
    ...
    await expect(page.locator('text=Avi yadhav')).not.toBeVisible();
    ...
    await expect(page.locator('text=Avi yadhav')).toBeVisible();
    await expect(page.locator('text=avi.yadhav+39@thinkitive.com')).toBeVisible();
  ...
  test('Restore Archived User', async ({ page }) => {
    ...
    await page.click('tr:has-text("Avi yadhav") button[aria-label="more"], tr:has-text("Snehal Bhor") button:has([data-testid="MoreVertIcon"])');
    await page.waitForTimeout(1000);
  ...
  test('Complete Archive and Restore Cycle', async ({ page }) => {
    ...
    // Archive the user
    await page.click('tr:has-text("Avi yadhav") button[aria-label="more"]');
    await page.waitForTimeout(1000);
  ...
  test('Archive Multiple Users and Restore Specific User', async ({ page }) => {
    ...
    const usersToTest = ['Avi', 'Test']; // Add more users as needed
    ...
    const snehalExists = await page.locator('text=Avi').isVisible();
    if (snehalExists) {
      await page.click('tr:has-text("Avi") button[aria-label="more"]');
      await page.waitForTimeout(1000);
      await page.click('li:has-text("Restore")');
      await page.waitForTimeout(3000);
